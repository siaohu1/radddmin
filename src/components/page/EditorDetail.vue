...
" @click='goBack'></i><span style="font-size: 20px;margin-left: 33%">编辑项目</span></div>
        <el-row class="rewBox" style="">
            <el-col :span="24"  v-show="infoCoinShow" >
                <el-form name="form"  class="formBox">
                    ...
                    <el-form-item >
                        <!--<el-button   @click="jumpicopage=true"></el-button>-->
                        <el-dialog
                            title="提示"
        ...
        methods: {
            getProjectDetail(projectId = this.$route.query.id){
                ...
                api.projectDetail({
                    ...
                    console.log(data);
                    //币标志
                    _this.symbol= data.projectName;
                    _this.bookUrl = data.whiteBook;
                    _this.desc = data.projectDesc;
                    _this.icoCost = data.icoCost;
                    _this.url= data.officialWebsite;
            ...
            saveDraft(projectId = this.$route.query.id){//保存草稿
                let _th = this;
                let tag= [];
                for (let i = 0; i < _th.tags.length; i++) {
                    tag.push(_th.tags[i].id);
            ...
            saveRelease(projectId = this.$route.query.id){//保存发布
                let _th = this;
                let tag= [];
                for (let i = 0; i < _th.tags.length; i++) {
                    tag.push(_th.tags[i].id);
